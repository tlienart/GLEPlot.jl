...
Clears all objects (annotations, arrows, ...) from the current axes, leaves
everything else.
"""
...
function axis(
    ...
    if !isempty(short)
        ...
        elseif s_lc == "equal"
            throw(
                NotImplementedError("axis(equal)")
            )
            #= would need to
...
function math(
    ...
    elseif s_lc == "off"
        ...
        throw(
            OptionValueError("Unrecognised shorthand toggle for math.", short)
        )
    end
    return PreviewFigure(gcf())
end
...
Set grid mode on. By default the grid will be associated with both `xticks`
and `yticks` but you can also specify one axis to only have horizontal or
vertical lines by passing `axis=["x"]`.
Options can be passed to specify the color of the grid, the style of the lines
or their width.
...
function grid(
    ...
    if !isempty(short)
        ...
        elseif s_lc == "on"
            ...
            throw(
                OptionValueError("Unrecognised shorthand toggle for grid.", short)
            )
        end
    end
    for ax ∈ axis
