...
"""
function legend(
            vd::Option{Vector{DrawingHandle{T}}} = nothing,
            labels::Option{Vector{String}}       = nothing;
            axes::Option{Axes}=nothing,
            opts...
        )::PreviewFigure where T

    axes isa Axes3D && throw(
        NotImplementedError("Legend for Axes3D")
    )
    ...
    # handles are given, check the labels
        ...
        isnothing(labels) && throw(
            ArgumentError("Labels must be provided along with handles")
        )
        length(vd) == length(labels) || throw(
            ArgumentError("There must be as many labels given as drawing handles")
        )
        axes.legend.handles = vd
        axes.legend.labels  = labels
    end
    set_properties!(axes.legend;  opts...)
